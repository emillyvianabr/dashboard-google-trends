...
function updateGeo(){
  ...
  charts.geo=new Chart(document.querySelector("#geoChart"),{
    ...
    options:{...standardOptions(),indexAxis:"y",plugins:{legend:{display:false},tooltip:{backgroundColor:"#1D232A",callbacks:{afterLabel:(ctx)=>isCoreCountry(data[ctx.dataIndex].country)?"Mercado monitorado":"Fora do recorte principal"}}},scales:{x:{grid:{color:"#EEF0F2"},border:{display:false},beginAtZero:true},y:{grid:{display:false},border:{display:false}}}}
  });
  updateGeoRadar();
}
