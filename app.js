...
function update(){updateKPIs();timeline();yoy();season();terms()}countries.forEach(()=>{});$('country').value='Mundo';$('start').value='';$('end').value='';['country','year','start','end'].forEach(id=>$(id).addEventListener('change',update));$('mapTopic').addEventListener('change',map);$('reset').onclick=()=>{$('country').value='Mundo';$('year').value='2026';$('start').value='';$('end').value='';update()};window.addEventListener('resize',()=>setTimeout(terms,150));update();map();
